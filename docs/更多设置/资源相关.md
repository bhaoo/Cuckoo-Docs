...
!!! info
    ...
    默认为 本地 ，若为 Dev 版本则推荐使用 本地 或 自定义CDN源 哦~
...
选择「Fastly」，即使用 Fastly 作为CDN进行获取静态资源

### Gcore

选择「Gcore」，即使用 Gcore 作为CDN进行获取静态资源

### JsdMirror (国内推荐)

选择「JsdMirror」，即使用 JsdMirror 作为CDN进行获取静态资源

### snrat (国内推荐)

选择「snrat」，即使用 snrat 作为CDN进行获取静态资源

### zstatic (国内推荐)

选择「zstatic」，即使用 zstatic 作为CDN进行获取静态资源
...
选择「dmoe.cc」，即使用 dmoe.cc 的随机图源API

### 自定义CDN源

选择「自定义CDN源」，即使用自定义的随机图源API，若出现图片完全重复可以在结尾加上 `?rand={rand}`

当选择 **本地** 并填写 **自定义CDN源** 时，可以实现从自定义的CDN源中读取随机图，但需本地 random 文件夹以及 自定义CDN源 均包含同一套随机图。
...
选择「WeAvatar」，即使用 WeAvatar 的 Gravatar 头像源API
